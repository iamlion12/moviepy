...
def slide_out(clip, duration, side):
    ...
    # Bug Fix: previous functions was incorrect
    pos_dict = {'right' : lambda t: (-w * min(ts-t, 0)/duration, 'center'),
                'left' : lambda t: (w * min(ts-t, 0)/duration, 'center'),
                'top' : lambda t: ('center', h*min(ts-t, 0)/duration),
                'bottom' : lambda t: ('center', -h*min(ts-t, 0)/duration)}
